...
public class Interpreter {
    ...
    private class ExprInterpreter implements IMC.Visitor<Long, Object> {
        ...
        public Long visit(IMC.BINOP imcBinop, Object arg) {
            ...
            if (debug)  System.out.printf("### %d %s %d\n", fstExpr, imcBinop.oper, sndExpr);
    ...
    private class StmtInterpreter implements IMC.Visitor<MEM.Label, StmtInterpreter.Info> {
        ...
        public MEM.Label visit(IMC.CJUMP imcCJump, Info arg) {
            if (debug){
                System.err.println("### CJUMP: " + imcCJump.cond);
                System.err.println("### CJUMP: " + imcCJump.posAddr + " -> " + imcCJump.negAddr);
                }   
        ...
        private void call(IMC.CALL imcCall) {
            Long offset = 0L;
            if (debug){
                System.err.println("CALL: " + imcCall);
                System.err.println("Offsets: " + imcCall.offs);
                System.err.println("Arguments: " + imcCall.args);
                System.err.println("Address: " + imcCall.addr);
            }
            for (IMC.Expr callArg : imcCall.args) {
                Long callValue = callArg.accept(new ExprInterpreter(this.interpreter), null);
                //System.err.println("CALL ARG: " + callValue);
            ...
            case IMC.TEMP t -> {
                ...
                System.err.println("TEMP Address: " + addr);
                if (addr != null) {
                    addrLabel = interpreter.labelsToAddr.get(addr.intValue());
                }
            }
